...
- For MVP time pressure, accept a light model: optional World ID metadata on each invoice — the contract stores a **normalized `address`** derived from the IDKit nullifier (`worldIdAddressFromNullifier`), not strong on-chain attestation until a later phase.
